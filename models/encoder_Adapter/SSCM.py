...
class HierarchicalVSSBlock(nn.Module):
    def __init__(self, hidden_dim, d_state=16, drop_path=0., **kwargs):
        ...
        self.self_attention = HierarchicalSS2D(d_model=hidden_dim, d_state=d_state, **kwargs)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.y_down = Downsampler(in_channels=64, out_channels=256)

    def forward(self, semantic_x, structural_x):
        shortcut = semantic_x
        semantic_x = self.ln_1(semantic_x.permute(0, 2, 3, 1).contiguous())
        structural_x = self.y_down(structural_x)
