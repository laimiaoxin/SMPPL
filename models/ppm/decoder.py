...
class HierMaskDecoder(nn.Module):
    def __init__(
        ...
        self.mask_tokens1 = nn.Embedding(self.num_mask_tokens, embed_dim)
        ...
        self.boundary_upscaling = nn.Sequential(
    ...
    def forward(
        ...
        mask_embeds = mask_embeds[:, mask_slice, :, :]
    ...
    def predict_masks(
        ...
        mask_tokens = self.mask_tokens1.weight.unsqueeze(0).expand(sparse_prompt_embeddings.size(0), -1, -1)
        ...
        if up_embeds != None:
            up_embeds = self.boundary_upscaling(up_embeds)
